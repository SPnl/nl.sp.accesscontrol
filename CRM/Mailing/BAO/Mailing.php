...
/*
 ...
 +--------------------------------------------------------------------+
 | 2015-08-20: Jaap Jansma (jaap.jansma@civicoop.org): Issue #468     |
 |             Toegangsrechten op mailings voor afdelingsgebruikers   |
 |                                                                    |
 +--------------------------------------------------------------------+
 */
...
";
  ...
  public static function mailingACLIDs() {
    ...
    if (!empty($groups)) {
       ...
       * BEGIN ISSUE #468
       *
       * Issue #468 (https://redmine.sp.nl/issues/468)
       * 2015-08-20: Jaap Jansma (jaap.jansma@civicoop.org)
       *
       * Standaard zorgt Civimail ervoor dat gebruikers mailings zien waarvan de groep verwijderd is.
       * Dit is bij de SP onhandig want daardoor worden Mailings zichtbaar afdelingsgebruikers die eigenlijk
       * verborgen hadden moeten blijven.
       */
 ...
 WHERE ( ( g.entity_table like 'civicrm_group%' AND g.entity_id IN ( $groupIDs ) )
  ...
  public static function commonCompose(&$form) {
    ...
    foreach ($modePrefixes as $prefix) {
      if ($prefix == 'SMS') {
        $templates[$prefix] = CRM_Accesscontrol_MessageTemplates_PermissionToChangeTemplate::getRestrictedTemplates(FALSE, TRUE);
      }
      else {
        $templates[$prefix] = CRM_Accesscontrol_MessageTemplates_PermissionToChangeTemplate::getRestrictedTemplates(FALSE);
      }
